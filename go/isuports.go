...
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	if err := adminDB.SelectContext(
		ctx,
		&vhs,
		"SELECT player_id, min_created_at FROM visit_history_first_created_at WHERE tenant_id = ? AND competition_id = ?",
		tenantID,
		comp.ID,
...
func competitionRankingHandler(c echo.Context) error {
	...
	if _, err := adminDB.ExecContext(
		ctx,
		"INSERT IGNORE INTO visit_history_first_created_at (player_id, tenant_id, competition_id, min_created_at) VALUES (?, ?, ?, ?)",
		v.playerID, tenant.ID, competitionID, now,
	); err != nil {
		return fmt.Errorf(
			"error Insert visit_history_first_created_at: playerID=%s, tenantID=%d, competitionID=%s, createdAt=%d, %w",
